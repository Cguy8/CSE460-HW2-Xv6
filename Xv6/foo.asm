  ...
  19:	83 39 01             	cmpl   $0x1,(%ecx)
{
  ...
  if(argc < 2 )
  1f:	0f 8f 80 00 00 00    	jg     a5 <main+0xa5>
    n = 1;       //default value
  25:	31 db                	xor    %ebx,%ebx
  27:	eb 2f                	jmp    58 <main+0x58>
  ...
  30:	0f 84 8e 00 00 00    	je     c4 <main+0xc4>
      printf(1, "Parent %d creating child  %d\n", getpid(), id );
  36:	e8 86 03 00 00       	call   3c1 <getpid>
  3b:	56                   	push   %esi
  for ( k = 0; k < n; k++ ) {
  3c:	83 c3 01             	add    $0x1,%ebx
      printf(1, "Parent %d creating child  %d\n", getpid(), id );
  3f:	50                   	push   %eax
  40:	68 6c 08 00 00       	push   $0x86c
  45:	6a 01                	push   $0x1
  47:	e8 a4 04 00 00       	call   4f0 <printf>
      wait ();
  4c:	e8 f8 02 00 00       	call   349 <wait>
  51:	83 c4 10             	add    $0x10,%esp
  for ( k = 0; k < n; k++ ) {
  54:	39 df                	cmp    %ebx,%edi
  56:	7e 30                	jle    88 <main+0x88>
    id = fork ();
  58:	e8 dc 02 00 00       	call   339 <fork>
  5d:	89 c6                	mov    %eax,%esi
    if ( id < 0 ) {
  5f:	85 c0                	test   %eax,%eax
  61:	79 cd                	jns    30 <main+0x30>
      printf(1, "%d failed in fork!\n", getpid() );
  63:	e8 59 03 00 00       	call   3c1 <getpid>
  ...
  88:	d9 ee                	fldz   
  8a:	dd 05 a0 08 00 00    	fldl   0x8a0
  90:	dc c1                	fadd   %st,%st(1)
  92:	d9 05 a8 08 00 00    	flds   0x8a8
  ...
  exit();
  a0:	e8 9c 02 00 00       	call   341 <exit>
    n = atoi ( argv[1] ); //from command line
  a5:	83 ec 0c             	sub    $0xc,%esp
  a8:	ff 70 04             	pushl  0x4(%eax)
  ab:	e8 20 02 00 00       	call   2d0 <atoi>
  if ( n < 0 || n > 20 )
  b0:	83 c4 10             	add    $0x10,%esp
    n = atoi ( argv[1] ); //from command line
  b3:	89 c7                	mov    %eax,%edi
  if ( n < 0 || n > 20 )
  b5:	83 f8 14             	cmp    $0x14,%eax
  b8:	76 22                	jbe    dc <main+0xdc>
    n = 2;
  ba:	bf 02 00 00 00       	mov    $0x2,%edi
  bf:	e9 61 ff ff ff       	jmp    25 <main+0x25>
      printf(1, "Child %d created\n",getpid() );
  c4:	e8 f8 02 00 00       	call   3c1 <getpid>
  c9:	52                   	push   %edx
  ca:	50                   	push   %eax
  cb:	68 8a 08 00 00       	push   $0x88a
  d0:	6a 01                	push   $0x1
  d2:	e8 19 04 00 00       	call   4f0 <printf>
      break;
  d7:	83 c4 10             	add    $0x10,%esp
  da:	eb ac                	jmp    88 <main+0x88>
  for ( k = 0; k < n; k++ ) {
  dc:	85 c0                	test   %eax,%eax
  de:	0f 85 41 ff ff ff    	jne    25 <main+0x25>
  e4:	eb a2                	jmp    88 <main+0x88>
  e6:	66 90                	xchg   %ax,%ax
  e8:	66 90                	xchg   %ax,%ax
  ea:	66 90                	xchg   %ax,%ax
  ec:	66 90                	xchg   %ax,%ax
 ...
 436:	89 d3                	mov    %edx,%ebx
{
 ...
 479:	0f b6 92 b4 08 00 00 	movzbl 0x8b4(%edx),%edx
...
printf(int fd, char *fmt, ...)
{
 ...
 674:	eb 1a                	jmp    690 <printf+0x1a0>
          s = "(null)";
 676:	bb ac 08 00 00       	mov    $0x8ac,%ebx
        while(*s != 0){
...
free(void *ap)
{
 ...
 6c1:	a1 6c 0b 00 00       	mov    0xb6c,%eax
{
 ...
 6e6:	73 10                	jae    6f8 <free+0x38>
{
 ...
 711:	5b                   	pop    %ebx
  freep = p;
 712:	a3 6c 0b 00 00       	mov    %eax,0xb6c
}
 717:	5e                   	pop    %esi
 718:	5f                   	pop    %edi
 719:	5d                   	pop    %ebp
 ...
 737:	03 53 fc             	add    -0x4(%ebx),%edx
  freep = p;
 73a:	a3 6c 0b 00 00       	mov    %eax,0xb6c
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 759:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 75c:	8b 3d 6c 0b 00 00    	mov    0xb6c,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 7a9:	8b 3d 6c 0b 00 00    	mov    0xb6c,%edi
 ...
 7d2:	e8 e9 fe ff ff       	call   6c0 <free>
  return freep;
 7d7:	8b 15 6c 0b 00 00    	mov    0xb6c,%edx
 ...
 804:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
 807:	89 15 6c 0b 00 00    	mov    %edx,0xb6c
}
 ...
 81f:	90                   	nop
    base.s.ptr = freep = prevp = &base;
 820:	c7 05 6c 0b 00 00 70 	movl   $0xb70,0xb6c
 827:	0b 00 00 
    base.s.size = 0;
 82a:	bf 70 0b 00 00       	mov    $0xb70,%edi
    base.s.ptr = freep = prevp = &base;
 82f:	c7 05 70 0b 00 00 70 	movl   $0xb70,0xb70
 836:	0b 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 839:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 83b:	c7 05 74 0b 00 00 00 	movl   $0x0,0xb74
 842:	00 00 00 
    if(p->s.size >= nunits){
